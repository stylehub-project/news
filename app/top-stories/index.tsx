...
const TopStoriesPage = () => {
  ...
  // Micro-Tour Check - Fixed Loop with Local Storage Persistance
  useEffect(() => {
      const TOUR_KEY = 'nc_tour_top_headlines_seen';
      const hasSeen = localStorage.getItem(TOUR_KEY);

      // Only start if not loading, has data, NOT seen before, and tour engine not busy
      if (!loading && articles.length > 0 && !hasSeen && !runTour) {
          // Immediately mark as seen to prevent re-triggering on reload/remount
